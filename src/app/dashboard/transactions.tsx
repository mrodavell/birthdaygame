    ...
    return (
        <SafeAreaView style={{ flex: 1, flexGrow: 1, flexDirection: 'column', paddingHorizontal: 10, marginTop: 10, marginBottom: bottom, justifyContent: 'flex-start' }}>
            <View style={{ flex: 1, padding: 10, marginTop: 10 }}>
                ...
                <Divider style={{ height: 1, marginHorizontal: 10, marginTop: 10 }} />
                <ScrollView style={{ maxHeight: heightScale(screenHeight), marginHorizontal: widthScale(5) }}>
                    {transactions.length === 0 &&
                        <View style={{ minHeight: heightScale(dimensions.height * 0.5), flex: 1, justifyContent: 'center', alignItems: 'center' }}>
                            <Text style={{ fontSize: moderateWs(20, 1) }}>No transactions to view</Text>
                        </View>
                    }
                    {transactions.reverse().map((value, index) => {
                        return <TouchableOpacity activeOpacity={1} key={`results-${index}`} style={{ marginTop: heightScale(15) }}>
                            <View>
                                <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'flex-start' }}>
                                    <MaterialCommunityIcons name='calendar' size={18} style={{ marginRight: 8 }} />
                                    <Text style={{ fontSize: moderateWs(12, 1), marginBottom: 5 }}>{dayjs(value.date).format('DD-MMM-YYYY h:m A')}</Text>
                                </View>
                                <List.Item
